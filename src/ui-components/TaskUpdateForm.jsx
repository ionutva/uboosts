...
export default function TaskUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...taskRecord };
    setId(cleanValues.id);
    setAid(cleanValues.aid);
    ...
    setIsImportant(cleanValues.isImportant);
    setErrors({});
  };
  const [taskRecord, setTaskRecord] = React.useState(task);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(Task, id) : task;
